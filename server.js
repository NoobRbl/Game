...
// Damage rules you requested:
const SKILL_DMG_MIN = 1209;
const SKILL_DMG_MAX = 2290;

// lighter + less lag settings
const TICK_HZ = 60;
const STATE_HZ = 20;
...
function Fighter(side, rng) {
  ...
  return {
    ...
    // chosen variants random:
    atkVar: 0,
    skillVar: 0,
    atkT: 0,   // timer inside attack
    actT: 0,   // timer inside current action
    didHit: false,
  };
}

// separation so they don't "hút" vào nhau
...
function rectHit(att, def, range, w, h) {
  ...
  const hy = -86; // hitbox center relative floor
  const dx = def.x;
  const dy = -86;
...
function rollCrit(rng) {
  // keep your 15% crit idea but skills are already high; crit makes it feel spicy.
  const isCrit = rng.f() < 0.15;
  return isCrit;
}
...
function startAttack(f, rng) {
  ...
  f.atkT = 0;
  f.didHit = false;
  f.atkVar = rng.int(0, 3); // 4 kiểu chém
  return true;
}
...
function startDash(f, dir) {
  ...
  f.face = d;
  f.vx = d * 2200; // faster dash as you asked
  f.vy *= 0.10;
...
function startSkill(f, which, rng) {
  ...
  if (which === "s1") {
    ...
    f.en = 0;
    f.cdUlt = 3.2;
    f.lock = 0.55; // shorter cinematic => less lag
    f.state = "ult";
  }

  f.actT = 0;
  f.didHit = false;
  f.skillVar = rng.int(0, 2); // 3 biến thể skill random
  return true;
}
...
function stepRoom(room, dt) {
  ...
  function updateF(f, other, inp) {
    ...
    // actions (simple edge-less: server uses "pressed" bool; client sends true only on press)
    ...
    // movement (can move while atk but slower + khựng nhẹ)
    let slow = 1;
    ...
    // hit windows (super-light)
    if (f.hp <= 0) { f.state = "dead"; return; }
    if (f.stun > 0) { f.state = "stun"; }

    // attack hit timing random per variant
    if (f.state === "atk") {
      f.atkT += dt;
      const roll = rollAtkDamage(rng);

      // each variant: different range feel
      const table = [
    ...
    // skills hit timing + dmg 1209..2290
    if (f.state === "s1" || f.state === "s2" || f.state === "sub" || f.state === "ult") {
      f.actT += dt;

      // 3 skill variants: (1) long-range slash, (2) close burst, (3) step-in
      const sv = f.skillVar;

      if (!f.didHit) {
        if (f.state === "ult") {
          // quick cinematic: 2 hits
          ...
          // s1/s2/sub: one main hit
          if (f.actT >= 0.08 && f.actT <= 0.14) {
            // little movement depending variant
            if (sv === 2) f.x += f.face * 120 * dt; // step-in tiny
  ...
  // reset inputs that were "pressed" (server treats them as one-shot)
...
setInterval(() => {
  ...
  for (const room of rooms.values()) {
    // only simulate when both connected
    if (!room.host || !room.guest) continue;
    stepRoom(room, dt);

    // send state at STATE_HZ
    room._stateAcc += dt;
    ...
    if (room.L.hp <= 0 || room.R.hp <= 0) {
      const keepSeed = (Math.random() * 0xFFFFFFFF) >>> 0;
      room.seed = keepSeed;
      room.rng = makeRng(keepSeed);
      room.L = Fighter(-1, room.rng);
      room.R = Fighter(+1, room.rng);
      room.tick = 0;
      broadcast(room, { t: "start", seed: keepSeed });
    }
  }
...
wss.on("connection", (ws) => {
  ws.on("message", (buf) => {
    ...
    if (msg.t === "in") {
      ...
      // IMPORTANT: treat actions as one-shot; movement always updated
      if (room.host === ws) {
        room.inL.mx = inp.mx;
